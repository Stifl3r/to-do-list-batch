...
import za.co.learnings.todolistbatch.batch.util.property.JobPropertyResolvers;
...
public class AdHocStarter {
    ...
    public AdHocStarter(JobLocator jobLocator,
                        JobRepository jobRepository,
                        JobPropertyResolvers jobPropertyResolvers,
                        @Value("${za.co.learnings.todolistbatch.batch.addUniqueJobParameter:true}") boolean addUniqueJobParameter) {
    ...
    public JobExecution start(JobConfig jobConfig) {
        try {
            var job = jobLocator.getJob(jobConfig.getName());
            ...
            var jobLauncher = jobConfig.isAsynchronous() ? asyncJobLauncher : syncJobLauncher;
